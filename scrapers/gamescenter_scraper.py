...
class GamescenterScraper:
    ...
    def _normalize_item_name(self, nombre: str) -> str:
        ...
        - "Free Fire - Tarjeta Semanal – Latinoamérica" → "Tarjeta Semanal"
        - "Free Fire – Pase Booyah Premium" → "Pase Booyah Premium"
        """
        try:
            # Remover prefijos: "Free Fire: ", "Free Fire - ", "Free Fire – "
            nombre = re.sub(r'^free\s+fire\s*[:\-–]\s*', '', nombre, flags=re.IGNORECASE)
            
            # Remover sufijos: " - Perú", " - Latinoamérica", " – Perú", " – Latinoamérica"
            nombre = re.sub(r'\s*[–\-]\s*(Perú|Latinoamérica)\s*$', '', nombre)
